package tw.ispan.iii.controller;
...
public class MyController3 {
	...
	@Autowired
	private ProductOrderService poService;
	...
	public String processPaymentResult2(HttpServletRequest request) {		
		...
		if ("1".equals(dict.get("RtnCode")) && checkStatus==true ){
			...
			//---------------------------//
			//回應用戶端(付款者)
			poService.findById(oderid);
			ProductOrder productOrder = poService.findById(oderid);;

			productOrder.setOrderstatus("已付款");
